...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    s = start
    while s % 10 != 0 and s <= stop:
      s += 1
    if s > stop :
      x = torch.empty(0, dtype = torch.float64)
    else :
      x = torch.arange(s, stop, 10, dtype = torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    c = x.clone()
    y1 = c.view(6, 4).t().contiguous().view(8, 3).t()
    idx = [0, 2, 4, 6, 1, 3, 5, 7]
    y = y1[:, idx]
    ##########################################################################
    #                            END OF YOUR CODE                            #
